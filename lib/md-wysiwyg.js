...
function normalizeLinkAnchor(anchor) {
  try {
    return decodeURIComponent(String(anchor || ''));
  } catch (_err) {
    return String(anchor || '');
  }
}

function slugifyMarkdownAnchor(text) {
  return normalizeLinkAnchor(text)
    .replace(/^#+/, '')
    .trim()
    .toLowerCase()
    .replace(/<[^>]*>/g, '')
...
function findHeadingPositionForAnchor(doc, anchor) {
  const normalizedAnchor = normalizeMarkdownLine(normalizeLinkAnchor(anchor)).toLowerCase();
  const anchorSlug = slugifyMarkdownAnchor(anchor);
  if (!anchorSlug && !normalizedAnchor) return 1;

  let best = null;
  doc.descendants((node, pos) => {
    if (best != null || !node.type || node.type.name !== 'heading') return true;
    const text = node.textContent || '';
    const normalizedText = normalizeMarkdownLine(text).toLowerCase();
    const slug = slugifyMarkdownAnchor(text);
    if (
      (anchorSlug && slug === anchorSlug) ||
      (normalizedAnchor && normalizedText === normalizedAnchor)
    ) {
      best = pos + 1;
      return false;
    }
...
function findSourceHeadingPositionForAnchor(markdown, anchor) {
  const normalizedAnchor = normalizeMarkdownLine(normalizeLinkAnchor(anchor)).toLowerCase();
  const anchorSlug = slugifyMarkdownAnchor(anchor);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const normalizedHeading = normalizeMarkdownLine(heading).toLowerCase();
    const slug = slugifyMarkdownAnchor(heading);
    if (
      (anchorSlug && slug === anchorSlug) ||
      (normalizedAnchor && normalizedHeading === normalizedAnchor)
    ) {
      return [i, Math.max(lines[i].search(/\S/), 0)];
    }
  }

  return findSourcePositionForAnchor(markdown, anchor);
}
...
function splitLinkHref(href) {
  ...
  if (hashIndex < 0) return { target: raw, anchor: '' };
  return {
...
class MdWysiwygEditor {
  ...
  async _init(filePath) {
    ...
    try {
      ...
      const editor = await kit.Editor.make()
        ...
        .use(kit.htmlCompatibilityPlugin)
  ...
  _bindEditorUiEvents() {
    ...
    const handleLinkClick = (event) => {
      const target = event.target && event.target.closest
        ...
        : null;
      if (!target || !this.editorContainer.contains(target)) return;
    ...
    this.editorContainer.addEventListener('click', handleLinkClick, true);
    this.disposables.add({
      dispose: () => {
        if (this.editorContainer) {
          this.editorContainer.removeEventListener('click', handleLinkClick, true);
        }
      },
    });
  }
  ...
  _showLinkPopover(link, focusHref) {
    ...
    this.linkPopover.style.visibility = 'hidden';

    try {
      const pos = link ? link.from : view.state.selection.from;
      const coords = view.coordsAtPos(pos);
      const margin = 12;
      const popoverRect = this.linkPopover.getBoundingClientRect();
      const width = popoverRect.width || 720;
      ...
      this.linkPopover.style.left = clamp(coords.left, margin, maxLeft) + 'px';
      this.linkPopover.style.top = clamp(top, margin, Math.max(margin, window.innerHeight - height - margin)) + 'px';
    } catch (_err) {
      this.linkPopover.style.left = '16px';
      this.linkPopover.style.top = '48px';
    }

    this.linkPopover.style.visibility = '';
    if (focusHref) {
      requestAnimationFrame(() => {
  ...
  async _openLocalLink(href) {
    const { target, anchor } = splitLinkHref(href);
    ...
    try {
      if (target) {
        if (target.startsWith('file://')) {
          ...
          targetPath = path.isAbsolute(target)
            ? target
            : path.resolve(path.dirname(this.filePath), normalizeLinkAnchor(target));
        }
      }

      if (!targetPath || !fs.existsSync(targetPath)) {
        atom.notifications.addWarning('Link target not found', { detail: href });
        return;
      }
      ...
      const opened = path.resolve(targetPath) === path.resolve(this.filePath)
        ? this
        : await atom.workspace.open(targetPath, { activateItem: true, searchAllPanes: true });

      if (opened && opened instanceof MdWysiwygEditor && anchor) {
        requestAnimationFrame(() => opened.navigateToAnchor(anchor));
      } else if (opened && opened instanceof MdWysiwygEditor) {
        opened.focus();
      } else if (anchor) {
        this._jumpTextEditorToAnchor(opened, anchor);
      }
    } catch (err) {
      atom.notifications.addError('Failed to open link', { detail: err.message });
    }
  ...
  _jumpToOutlineItem(index) {
    ...
    try {
      ...
      this._selectAndScrollToPos(view, pos, { smooth: true });
      this._updateOutlineActiveItem();
    } catch (err) {
  ...
  _selectAndScrollToPos(view, pos, options = {}) {
    if (!view || !milkdownModule.TextSelection) return;
    const selection = milkdownModule.TextSelection.near(view.state.doc.resolve(pos));
    view.dispatch(view.state.tr.setSelection(selection));
    view.focus();
    this._scrollEditorToPos(view, pos, options);
  }

  _scrollEditorToPos(view, pos, options = {}) {
    const scroller = this._getScrollElement();
    if (!scroller || !view || typeof view.coordsAtPos !== 'function') return;

    requestAnimationFrame(() => {
      try {
        const coords = view.coordsAtPos(pos);
        const scrollerRect = scroller.getBoundingClientRect();
        const offset = Math.max(24, Math.round(scroller.clientHeight * 0.16));
        const maxScroll = Math.max(0, scroller.scrollHeight - scroller.clientHeight);
        const targetTop = clamp(
          scroller.scrollTop + coords.top - scrollerRect.top - offset,
          0,
          maxScroll
        );
  ...
  navigateToAnchor(anchor, attempts = 20) {
    if (!anchor) return;
    const view = this._getEditorView();
    if (!view || !milkdownModule.TextSelection) {
      if (attempts > 0) {
        setTimeout(() => this.navigateToAnchor(anchor, attempts - 1), 50);
      }
      return;
    }

    try {
      const found = findHeadingPositionForAnchor(view.state.doc, anchor);
      if (found == null) {
        atom.notifications.addWarning('Heading not found', { detail: '#' + anchor });
        return;
      }
      const pos = clamp(found, 1, Math.max(view.state.doc.content.size - 1, 1));
      this._selectAndScrollToPos(view, pos, { smooth: true });
      this._updateOutlineActiveItem();
    } catch (err) {
      atom.notifications.addError('Failed to jump to heading', { detail: err.message });
    }
  }

  _jumpTextEditorToAnchor(textEditor, anchor) {
    if (!textEditor || !anchor || !textEditor.getText) return;
    const position = findSourceHeadingPositionForAnchor(textEditor.getText(), anchor);
    if (!position) {
      atom.notifications.addWarning('Heading not found', { detail: '#' + anchor });
      return;
    }
    if (textEditor.setCursorBufferPosition) {
      textEditor.setCursorBufferPosition(position, { autoscroll: false });
    }
    if (textEditor.scrollToBufferPosition) {
...
module.exports = {
  ...
  async pasteClipboard() {
    const active = atom.workspace.getActivePaneItem();
    if (!(active instanceof MdWysiwygEditor)) return;
    const imageAttrs = await this._readClipboardImage(active.getPath());
    if (imageAttrs) {
      active._runEditorCommand('insertImage', { ...imageAttrs, select: true });
      return;
    }
  ...
  async _readClipboardImage(docPath) {
    let clipboard;
    try {
    ...
    try {
      return await imageAssets.localizeImageBuffer(png, docPath, {
        name: 'clipboard-image.png',
        type: 'image/png',
      });
    } catch (err) {
      atom.notifications.addError('Failed to paste clipboard image', { detail: err.message });
      return null;
    }
  },
  ...
  _renderImageAssetManager(root, index, close) {
    ...
    const title = document.createElement('div');
    title.className = 'md-wysiwyg-asset-title';
    ...
    const usedCount = index.assets.filter((asset) => asset.references.length > 0).length;
    const unusedCount = index.assets.length - usedCount;
    const summary = document.createElement('em');
    summary.textContent = index.assets.length + ' images · ' + usedCount + ' referenced · ' +
      unusedCount + ' unused';
    ...
    cleanButton.title = 'Delete unused image assets';
    cleanButton.disabled = unusedCount === 0;
    cleanButton.addEventListener('click', async () => {
      ...
      try {
        ...
        this._notifyImageAssetsChanged();
        await refresh();
      } catch (err) {
    ...
    index.assets.forEach((asset) => {
      ...
      preview.addEventListener('click', () => this._showImagePreview(asset, root));
      ...
      if (asset.references.length === 0) {
        ...
        unused.className = 'md-wysiwyg-asset-badge unused';
        unused.textContent = 'Unused';
        refs.appendChild(unused);
      } else {
        const badge = document.createElement('span');
        badge.className = 'md-wysiwyg-asset-badge';
        badge.textContent = asset.references.length + ' ref' + (asset.references.length === 1 ? '' : 's');
      ...
      copyButton.title = 'Copy the image path for the active document';
      copyButton.addEventListener('click', () => {
        ...
        atom.notifications.addSuccess('Image path copied');
      });

      const deleteButton = document.createElement('button');
      deleteButton.type = 'button';
      deleteButton.className = 'btn btn-xs md-wysiwyg-asset-delete';
      deleteButton.textContent = 'Delete';
      deleteButton.title = 'Delete this file from the image assets directory';
      deleteButton.addEventListener('click', async () => {
        if (asset.references.length > 0) {
          this._showAssetReferences(asset, refresh);
          return;
        }
        await this._confirmAndDeleteAsset(asset, refresh);
      });
  ...
  async _confirmAndDeleteAsset(asset, refresh) {
    const choice = atom.confirm({
      message: 'Delete image asset?',
      detailedMessage: asset.filePath,
      buttons: ['Delete', 'Cancel'],
    });
    if (choice !== 0) return false;
    try {
      await imageAssets.deleteAssets([asset.filePath]);
      atom.notifications.addSuccess('Image asset deleted');
      this._notifyImageAssetsChanged();
      if (refresh) await refresh();
      return true;
    } catch (err) {
      atom.notifications.addError('Failed to delete image asset', { detail: err.message });
      return false;
    }
  },

  _showAssetReferences(asset, refresh) {
    const root = document.createElement('div');
    root.className = 'md-wysiwyg-asset-references-modal';
    const title = document.createElement('strong');
    title.textContent = 'Image is still referenced';
    const message = document.createElement('p');
    message.textContent = 'This image is referenced by the following Markdown document' +
      (asset.references.length === 1 ? '.' : 's.') +
      ' You can still force delete it.';
    const pathText = document.createElement('code');
    pathText.textContent = asset.filePath;
    const list = document.createElement('div');
    list.className = 'md-wysiwyg-asset-reference-list';

    asset.references.forEach((ref) => {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'btn';
      button.textContent = path.basename(ref.docPath);
      button.title = ref.docPath;
      button.addEventListener('click', () => atom.workspace.open(ref.docPath));
    ...
    const deleteButton = document.createElement('button');
    deleteButton.type = 'button';
    deleteButton.className = 'btn btn-error md-wysiwyg-asset-force-delete';
    deleteButton.textContent = 'Delete';
    ...
    const panel = atom.workspace.addModalPanel({ item: root, visible: true });
    deleteButton.addEventListener('click', async () => {
      const deleted = await this._confirmAndDeleteAsset(asset, refresh);
      if (deleted) panel.destroy();
    });
    closeButton.addEventListener('click', () => panel.destroy());
  },

  _showImagePreview(asset, managerRoot) {
    const root = document.createElement('div');
    root.className = 'md-wysiwyg-image-preview-overlay';
    root.addEventListener('mousedown', (event) => event.stopPropagation());
    root.addEventListener('click', (event) => event.stopPropagation());
    const modal = document.createElement('div');
    modal.className = 'md-wysiwyg-image-preview-modal';
    const image = document.createElement('img');
    image.src = pathToFileURL(asset.filePath).href;
    image.alt = asset.name;
    ...
    (managerRoot || document.body).appendChild(root);
  },

  _notifyImageAssetsChanged() {
    window.dispatchEvent(new CustomEvent('md-wysiwyg:assets-changed'));
  },
