...
function previewUrlForSrc(src, docPath) {
  ...
  try {
    ...
    return pathToFileURL(localPath).href + '?mtime=' + encodeURIComponent(String(stat.mtimeMs));
  } catch (_err) {
    return '';
  }
}
...
async function localizeImageBuffer(buffer, docPath, options = {}) {
  if (!buffer || !docPath) return null;
  const assetsDir = await ensureAssetsDirectory();
  const fallbackExt = extensionForMime(options.type || 'image/png');
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  const sourceName = options.name || 'clipboard-image-' + stamp + fallbackExt;
  const target = uniqueTargetPath(assetsDir, sourceName, fallbackExt);
...
function collectImageReferences(markdown, docPath) {
  ...
  while ((match = pattern.exec(markdown))) {
    ...
    const key = path.resolve(localPath) + '\0' + path.resolve(docPath);
...
async function scanMarkdownReferences() {
  ...
  if (typeof atom !== 'undefined' && atom.workspace && atom.workspace.getPaneItems) {
    atom.workspace.getPaneItems().forEach((item) => {
      if (!item || !item.getPath || !item.getMarkdownContent) return;
      const docPath = item.getPath();
      if (!isMarkdownFile(docPath)) return;
      try {
        references.push(...collectImageReferences(item.getMarkdownContent(), docPath));
      } catch (_err) {
        // Open WYSIWYG items can still be indexed from disk if serialization fails.
      }
    });
  }
...
function dedupeReferences(references) {
  ...
  return references.filter((ref) => {
    const key = path.resolve(ref.localPath) + '\0' + path.resolve(ref.docPath);
